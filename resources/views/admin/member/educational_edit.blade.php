...
<div class="card m-3">
    ...
    <div class="card-body">
        @if($user->educationInfo && $user->educationInfo->count() > 0)
        @foreach ($user->educationInfo as $item)
            <div class="card">
                <div class="card-body">
                    <form action="{{ url('update/educationalInfo/'.$item->id) }}" method="POST">
                        @csrf
                        <div class="row">
        ...
        @else 
        <div class="alert alert-danger">This user have not Education info to edit</div>
        @endif
    </div>
</div>
